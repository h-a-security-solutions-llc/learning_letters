<template>
  <div class="selection-container">
    ...
    <div class="instructions">
      ...
      <button
        v-if="isAuthenticated && hasAnyProgress"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const getCharProgress = (char) => {
      // Filter for freestyle mode only and find the entry with the highest stars
      ...
      // Return the entry with the highest stars
    ...
    const getStarsDisplay = (starCount) => {
      const filled = '★'
      const empty = '☆'
      const count = Math.min(5, Math.max(0, starCount))
    ...
    // Debug: watch progress changes
    watch(() => props.progress, (newProgress) => {
      if (newProgress && newProgress.length > 0) {
        console.log('Progress data received:', JSON.stringify(newProgress, null, 2))
      }
    }, { immediate: true })
    ...
    // Check if user has any progress to clear
    ...
    // Confirm and emit clear scores event
